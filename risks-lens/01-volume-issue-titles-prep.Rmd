...
make_issue_df <-function(p) {
  ...
  # test
  # p <- read_html('./data/pagecache/catless.ncl.ac.uk-Risks-12-index.html')
  ...
  d_issue_subj_href <- p %>%
    ...
    separate(tmp_issues_subj, into = c("volume", "issue"), sep = '/') %>%
    separate(issue, into = c("issue", "subj"), sep = '#') 
  
  d <- left_join(d_issue_subj_href, d_issue_subj_text, by = c("name")) %>% #"rowid", 
    select(-name) %>%
